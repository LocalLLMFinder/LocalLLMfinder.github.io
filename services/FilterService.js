...
export class FilterService {
  ...
  applyFilters(models, filterState) {
    ...
    if (filterState.quantizations && filterState.quantizations.length > 0) {
      filteredModels = filteredModels.filter(model => 
        filterState.quantizations.includes(model.quantization)
      );
    }
    ...
    if (filterState.architectures && filterState.architectures.length > 0) {
      filteredModels = filteredModels.filter(model => 
        filterState.architectures.includes(model.architecture)
      );
    }
    ...
    if (filterState.families && filterState.families.length > 0) {
      filteredModels = filteredModels.filter(model => 
        filterState.families.includes(model.family)
      );
    }
    ...
    if (filterState.sizeRanges && filterState.sizeRanges.length > 0) {
      filteredModels = filteredModels.filter(model => 
        this.matchesSizeRange(model.sizeBytes, filterState.sizeRanges)
      );
    }
  ...
  getAvailableOptions(models) {
    ...
    models.forEach(model => {
      ...
      // Collect families
      if (model.family && model.family !== 'Unknown') {
  ...
  getSuggestedSearchTerms(models, partialQuery = '') {
    ...
    models.forEach(model => {
      // Add model family suggestions
      if (model.family !== 'Unknown' && model.family.toLowerCase().includes(query)) {
        suggestions.add(model.family);
      }

      // Add architecture suggestions
      if (model.architecture !== 'Unknown' && model.architecture.toLowerCase().includes(query)) {
        suggestions.add(model.architecture);
      }

      // Add quantization suggestions
      if (model.quantization !== 'Unknown' && model.quantization.toLowerCase().includes(query)) {
        suggestions.add(model.quantization);
      }
    });
